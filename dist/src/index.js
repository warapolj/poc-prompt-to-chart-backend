...
import { formatDataForShadcn, generateShadcnChartResponse } from '../packages/chart-config.js';
...
async function generateSQLQueryWithAI(columnAnalysis, userQuery, availableColumns, tableName = 'olympic_medalists') {
    ...
    const columnsMetadata = availableColumns.map((col) => {
        const isNumeric = ['int', 'bigint', 'decimal', 'float', 'double'].includes(col.type.toLowerCase());
    ...
    const columnsInfo = columnsMetadata
        .map((col) => `${col.name} (${col.type}): ${col.comment} [Groupable: ${col.canBeGrouped}, Aggregatable: ${col.canBeAggregated}]`)
        .join('\n');
    // วิเคราะห์ chart type เพื่อกำหนดรูปแบบ query
    const chartQueryPatterns = {
        ...
        column: 'SELECT {category_column}, COUNT(*) AS count FROM {table} {where_clause} GROUP BY {category_column} ORDER BY count DESC {limit_clause}',
        ...
        histogram: 'SELECT {numeric_column}, COUNT(*) AS frequency FROM {table} {where_clause} GROUP BY {numeric_column} ORDER BY {numeric_column}',
        donut: 'SELECT {category_column}, COUNT(*) AS count FROM {table} {where_clause} GROUP BY {category_column} ORDER BY count DESC LIMIT 10',
    };
  ...
  `;
    try {
        ...
        const contentString = typeof sqlResult.content === 'string' ? sqlResult.content : JSON.stringify(sqlResult.content);
...
async function analyzeTableStructure(tableName) {
    ...
    `, [dbConfig.database, tableName]);
        return rows.map((row) => {
            ...
            const isText = ['varchar', 'text', 'char', 'longtext', 'mediumtext', 'tinytext'].includes(row.data_type.toLowerCase());
            return {
                ...
                canBeAggregated: isNumeric,
                suitableForFilter: true,
                suggestedChartTypes: getSuggestedChartTypes(row.column_name, row.data_type, row.column_comment),
            };
        });
    }
    catch (error) {
...
function getSuggestedChartTypes(columnName, dataType, comment) {
    const suggestions = [];
...
async function getAvailableColumns(tableName = 'olympic_medalists') {
    ...
    return streamSSE(c, async (stream) => {
        try {
            ...
            const columnsDescription = availableColumns
                ...
                const capabilities = [];
                if (col.canBeGrouped)
                    capabilities.push('groupable');
                if (col.canBeAggregated)
                    capabilities.push('aggregatable');
                if (col.suitableForFilter)
                    capabilities.push('filterable');
                return `- ${col.name} (${col.type}): ${col.comment || 'ไม่มีคำอธิบาย'} [${capabilities.join(', ')}]${chartTypes}`;
            })
                .join('\n      ');
      ...
      `;
            ...
            try {
                // แปลง content เป็น string และหา JSON
                const contentString = typeof analysisResult.content === 'string'
                ...
                const defaultGroupableColumns = availableColumns
                    ...
                    .slice(0, 2)
                    .map((col) => col.name);
                const defaultXAxis = availableColumns.filter((col) => col.canBeGrouped)[0]?.name || 'country';
                columnAnalysis = {
                    required_columns: defaultGroupableColumns.length > 0 ? defaultGroupableColumns : ['country', 'medal'],
                    chart_type: 'bar',
            ...
            await stream.writeSSE({
                data: JSON.stringify({
                    type: 'status',
                    message: `ผลการวิเคราะห์: ใช้ ${columnAnalysis.chart_type} chart กับ columns ${columnAnalysis.required_columns.join(', ')}`,
                    progress: 75,
                }),
                event: 'update',
            });
            ...
            const result = await generateSQLQueryWithAI(columnAnalysis, userQuery, availableColumns, 'olympic_medalists');
            const { sqlQuery, explanation, queryReasoning, columnsUsed, filtersApplied, chartSuitability, } = result;
            await stream.writeSSE({
                data: JSON.stringify({
                    type: 'status',
...
app.post('/api/query', async (c) => {
    ...
    const vectorStore = await MemoryVectorStore.fromDocuments(splitDocs, new GoogleGenerativeAIEmbeddings({
